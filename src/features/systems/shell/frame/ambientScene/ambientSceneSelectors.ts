...
import type {
  ...
  SceneWorld,
  VisualComponent,
} from '../../../../components/shell/frame/ambientScene/ambientSceneTypes'
...
const VISIBLE_BY_MODE: Readonly<
  Record<ExperienceMode, (visual: VisualComponent) => boolean>
> = {
  cinematic: () => true,
  quiet: (visual) => visual.priority === 'essential',
}
...
export const selectSceneEntities = (
  world: SceneWorld,
  mode: ExperienceMode
): readonly SceneEntity[] =>
  world.ids
    .map((id) => assembleEntity(world, id))
    .filter((entity) => VISIBLE_BY_MODE[mode](entity.visual))
...
export const selectAmbientSceneViewModel = (
  ...
  const entities = world ? selectSceneEntities(world, projection.mode) : []
  return {
    visible: projection.visible && Boolean(world),
    className: `orbital-scene orbital-scene-${projection.mode}`,
